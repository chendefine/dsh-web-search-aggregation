...
describe('queue ordering and fallback', () => {
  ...
  it('skips a disabled entry without touching its endpoint', async () => {
    ...
    const harness = new TestHarness(queue(
      { kind: 'tavily', enabled: false, apiKeyRefs: ['TAVILY_API_KEY'] },
      { kind: 'tinyfish', enabled: true, apiKeyRefs: ['TINYFISH_API_KEY'] },
    ))
    harness.keys.set('TINYFISH_API_KEY', 'k')
    const result = await harness.provider.search({ query: 'q' })
  ...
  it('reports WEB_PROVIDER_UNAVAILABLE when no entry is enabled', async () => {
    const harness = new TestHarness(queue({ kind: 'tavily', enabled: false, apiKeyRefs: [] }))
    const caught: unknown = await harness.provider.search({ query: 'q' }).then(value => value, reason => reason)
...
describe('key handling', () => {
  it('falls through a failed key to the next key within one entry', async () => {
    const seen = routeFetch([
      { match: /tavily\.com/, respond: () => new Response('{"detail":"quota"}', { status: 429 }) },
      { match: /tavily\.com/, respond: () => searchHit(['https://tavily.test/key2']) },
    ])
    const harness = new TestHarness(queue({ kind: 'tavily', enabled: true, apiKeyRefs: ['K1', 'K2'] }))
    harness.keys.set('K1', 'first')
    harness.keys.set('K2', 'second')
    const result = await harness.provider.search({ query: 'q' })
    expect(result.sources[0]?.url).toBe('https://tavily.test/key2')
    expect(seen).toHaveLength(2)
  })

  it('records a failed credential resolution and continues with the remaining keys', async () => {
    routeFetch([{ match: /tavily\.com/, respond: () => searchHit(['https://tavily.test/ok']) }])
    const harness = new TestHarness(queue({ kind: 'tavily', enabled: true, apiKeyRefs: ['MISSING', 'K2'] }))
    harness.keys.set('K2', 'second')
    const result = await harness.provider.search({ query: 'q' })
    expect(result.sources[0]?.url).toBe('https://tavily.test/ok')
  })

  it('attempts anysearch anonymously when no refs are configured', async () => {
    const seen = routeFetch([{ match: /anysearch\.com/, respond: () => searchHit(['https://any.test/anon']) }])
    const harness = new TestHarness(queue({ kind: 'anysearch', enabled: true, apiKeyRefs: [] }))
    const result = await harness.provider.search({ query: 'q' })
    expect(result.sources[0]?.url).toBe('https://any.test/anon')
    expect(seen).toHaveLength(1)
  })

  it('records keyless key-required entries as a failure and moves on', async () => {
    routeFetch([{ match: /anysearch\.com/, respond: () => searchHit(['https://any.test/ok']) }])
    const harness = new TestHarness(queue(
      { kind: 'tavily', enabled: true, apiKeyRefs: ['UNSET'] },
      { kind: 'anysearch', enabled: true, apiKeyRefs: [] },
    ))
    const result = await harness.provider.search({ query: 'q' })
    expect(result.sources[0]?.url).toBe('https://any.test/ok')
  })

  it('rotates the start key per served request (round-robin over healthy keys)', async () => {
    const seen: string[] = []
    ...
    const bodies = seen.map(url => url)
    expect(bodies).toHaveLength(3)
    // Rotation is asserted through the log's ref: first, second, first.
    const refs = harness.log.filter(entry => entry.level === 'info').map(entry => entry.message)
    expect(refs[0]).toContain('K1')
    expect(refs[1]).toContain('K2')
    expect(refs[2]).toContain('K1')
  })
})
...
describe('timeouts and cancellation', () => {
  it('cuts one attempt off at attemptTimeoutMs and falls through', async () => {
    ...
    const harness = new TestHarness({
      providers: [
        { kind: 'tavily', enabled: true, apiKeyRefs: ['K'] },
        { kind: 'tinyfish', enabled: true, apiKeyRefs: ['T'] },
      ],
      attemptTimeoutMs: 50,
    })
    harness.keys.set('K', 'k')
    harness.keys.set('T', 't')
    const result = await harness.provider.search({ query: 'q' })
  ...
  it('aborts the walk immediately when the caller signal is already aborted', async () => {
    ...
    const harness = new TestHarness(queue({ kind: 'tavily', enabled: true, apiKeyRefs: ['K'] }))
    harness.keys.set('K', 'k')
