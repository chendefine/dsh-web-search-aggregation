...
[DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness)（DSH）双端插件：把内置 `web_search` 工具的搜索后端换成**一条按优先级排序的聚合队列（AnySearch / TinyFish / Tavily / Brave / Exa / Firecrawl / Jina）**——每个提供商可挂多个 API key 按轮转使用，任一次尝试失败自动落到下一个 key 或下一个提供商，第一个成功者生效。
...
- **多 key 池 + 轮转** —— 每个提供商只读一个凭据（`ANYSEARCH_API_KEY` / `TINYFISH_API_KEY` / `TAVILY_API_KEY` / `BRAVE_SEARCH_API_KEY` / `EXA_API_KEY` / `FIRECRAWL_API_KEY` / `JINA_API_KEY`），其值为该提供商全部 key 用 `,` 拼接。同一条目内的 key 按轮转顺序尝试（每成功一次前进一格），在池内分摊负载与配额。
- **开箱即用** —— AnySearch 允许匿名访问，没配任何 key 时队列即可完成搜索。出厂默认只是启用全部七家——先后顺序完全由你在卡片里编排；需 key 的条目在凭据就位前只是跳过。
...
当前内置七个 adapter；接入下一个上游只需一个 adapter 模块加一行注册表。
...
- 至少一个上游可达：默认队列不配任何凭据即可用（AnySearch 匿名）；Tavily / TinyFish / Brave / Exa / Firecrawl / Jina 条目需配各自 API key 才会参与。
...
| 字段 | 默认 | 说明 |
| --- | --- | --- |
| `providers` | 七条启用条目——每种一家 | 优先级队列。每个条目：提供商（每种最多一次）、启用开关（停用条目保留配置但被跳过）、可选的端点 base URL（覆盖默认）。出厂顺序不代表任何含义，可随意编排。 |
...
pnpm test        # vitest run（89 个单元测试，不联网）
...
├── adapters/              # AnySearch / Tavily / TinyFish / Brave / Exa / Firecrawl / Jina adapter + 共享 HTTP
...
搜索词与 API key 只发往每个队列条目配置的端点（默认七家提供商的官方 API）。key 存于 DSH credentials 域——不落 settings 文件、不进日志、不回显客户端。失败记录与日志只引用打码标签（如 `TAVILY_API_KEY#2`），不含明文。除请求它的会话外，不保留任何结果数据。
